...
function StationClock(clockId) {
  ...
  this.bodyColor		 = elem.data('body-color');
  this.strokeColor       = elem.data('stroke-color');
  
  // body dial
  this.dial              = StationClock.GermanStrokeDial;
  this.dialColor         = elem.data('dial-color');
  ...
  this.handShadowBlur    = 0.04;
	...
	// clock colors
	this.hourHandColor     = elem.data('hour-color');
	this.minuteHandColor   = elem.data('minute-color');
	this.secondHandColor   = elem.data('second-color');
  ...
  this.bossColor	     = elem.data('boss-color');
  ...
  //Date
  this.dateColor = elem.data('date-color');
};
...
StationClock.prototype.draw = function() {
  ...
  if (clock) {
    ...
    if (context) {
	  var width = parseInt(clock.width);
	  var height = parseInt(clock.height);
      this.radius = 0.75 * (Math.min(width, height) / 2);
      ...
      if (this.body != StationClock.NoStrokeBody) {
        ...
        switch (this.body) {
    		  case StationClock.SmallWhiteBody:
    		    this.fillCircle(context, this.bodyColor, 0, 0, 1);
    		    break;
          case StationClock.RoundBody:
            this.fillCircle(context, this.bodyColor, 0, 0, 1.1);
            context.save();
            this.setShadow(context, this.bodyShadowColor, this.bodyShadowOffsetX, this.bodyShadowOffsetY, this.bodyShadowBlur);
            this.strokeCircle(context, this.strokeColor, 0, 0, 1.1, 0.07);
            context.restore();
            break;
      ...
      if (this.boss != StationClock.NoBoss) {
        ...
        switch (this.boss) {
    		  case StationClock.BlackBoss:
    		    this.fillCircle(context, this.bossColor, 0, 0, 0.1);
    		    break;
    		  case StationClock.RedBoss:
    		    this.fillCircle(context, this.bossColor, 0, 0, 0.06);
    		    break;
          case StationClock.ViennaBoss:
            this.fillCircle(context, this.bossColor, 0, 0, 0.07);
            break;
        }
        context.restore();
      }
		...
		var fontsize = 'bold '+width/12+'px Arial';
		var topposition = width / 5.8;
		
        this.fillText(context, this.dateColor, fontsize, date, 0, topposition);
		context.restore();
...
StationClock.prototype.fillText = function(context, color, fontsize, date, offsetX, offsetY) {
  if (color) {
	context.font = fontsize;
	context.textAlign = "center";
	context.fillStyle = color;
	context.fillText(date, offsetX, offsetY);
  }
};
