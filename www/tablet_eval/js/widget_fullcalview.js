...
var Modul_fullcalview = function () {
	...
	function init () {
      ...
      me.addReading(elem,'get');
			...
			var fhemHost 						= $("meta[name='fhemweb_url']").attr('content')?$("meta[name='fhemweb_url']").attr('content'):'';
			...
			((eventsources.length == eventcolors.length) && (eventsources.length == eventtextcolors.length)))
			{
				...
				for (var n=0; n < eventsources.length; n++) {
		          ...
		          $('#calendar').fullCalendar('addEventSource', {
			      		events: fc_events(this.response, eventcolors[clsn], eventtextcolors[clsn]),
			      	});
		          if (!e) {
							}
		      	});
  ...
  function update (dev,par) {
    var me = this;
		...
		for (var n=0; n < eventsources.length; n++) {
			if (eventsources[n] == dev)
			{
				for (var n=0; n < eventsources.length; n++) {
   				data_req('/fhem?detail='+eventsources[n]+'&dev.get'+eventsources[n]+'='+eventsources[n]+'&cmd.get'+eventsources[n]+'=get&arg.get'+eventsources[n]+'=vcalendar&val.get'+eventsources[n]+'=&XHR=1', function(e,r) {
